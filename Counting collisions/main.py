...
# TO-DO: insert the keys, in order, into all the hash tables, keeping track of
# the total number of collisions for that table.

for element in keys:
    result1, partial_lp_collisions = lp.insert(element)
    result2, partial_qp1_collisions = qp1.insert(element)
    result3, partial_qp2_collisions = qp2.insert(element)
    result4, partial_dh_collisions = dh.insert(element)

    lp_collisions += partial_lp_collisions
    qp1_collisions += partial_qp1_collisions
    qp2_collisions += partial_qp2_collisions
    dh_collisions += partial_dh_collisions
